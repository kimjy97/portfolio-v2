...
export const projectData: IProjectProps[] = [{
  ...
  issues: [
    ...
    {
      issue: '초기 웹 주식 게임 특성상 가입 후 실제 거래로 이어지지 않고 이탈하거나, 일회성 방문에 그치기 쉬운 문제',
      solving: '직관적인 온보딩 인터페이스와 **연속 출석 보상 시스템**을 구축하여 __가입자 대비 첫 거래 실행률 90.0%(18/20명)__, __출석 참여율 90.0%(최대 27일 연속 방문)__, __상점 아이템 구매 전환율 45.0%__라는 높은 참여를 이끌어냈습니다. 또한 인앱 설문조사로 __체결 만족도 100%__ 피드백을 확인하고, 유저 의견을 호가창 반응성 개선에 빠르게 반영했습니다.'
    }
  ],
  reason: "`Next.js 16 App Router`와 `React 19`로 모바일과 데스크톱 모두에서 실제 모바일 주식 앱처럼 부드럽게 동작하는 반응형 트레이딩 대시보드를 구축했고, 대량의 캔들 데이터를 끊김 없이 렌더링하기 위해 `TradingView Lightweight Charts`를 채택했습니다. 백엔드는 대규모 체결 엔진과 이벤트 처리에 적합한 `NestJS`, `PostgreSQL`, `Redis`를 사용했습니다. 24시간 가동되는 마켓 메이커 봇은 메인 서버와 분리하여 부하 영향을 차단했고, `Oracle Cloud ARM`과 `Docker` 기반 환경에서 경량 AI(`Ollama`)를 직접 서빙하여 외부 API 비용 없이 2~3초 만에 실시간 속보 뉴스를 발행하는 효율적인 시스템을 완성했습니다.",
  learned: ' 실시간 주식 거래 플랫폼을 밑바닥부터 설계하고 오픈베타를 운영하면서, **데이터베이스 인덱싱과 사전 집계만으로도 조회 속도를 8.6초에서 0.8ms로 극적으로 개선할 수 있다는 점**을 배웠습니다. 또한 대량의 주문이 몰릴 때 핵심 작업과 비동기 작업을 분리하여 데이터베이스 충돌을 방지하고 시스템 안정성을 지키는 방법을 깊이 체감했습니다.\n 제한된 클라우드 자원에서 **컨테이너 메모리를 효율적으로 분배해 서버 병목을 해결하고, 경량 온디바이스 AI 모델을 최적화해 빠른 응답 속도를 확보**하는 인프라 튜닝 역량을 길렀습니다. 아울러 **오픈베타 유저들의 90% 출석률을 이끌어낸 보상 시스템 설계와 실제 사용자 설문 피드백을 바탕으로 제품을 고도화**해보며, 기술뿐만 아니라 사용자 경험까지 아우르는 개발 시야를 갖추게 되었습니다.',
  intro: '자체 개발한 마켓 메이커 시뮬레이션 엔진과 실시간 체결 엔진을 탑재한 웹 기반 모의투자 주식 게임입니다. 실제 오픈베타 테스트(OBT)를 운영하며 수집한 유저 데이터를 바탕으로 서비스를 지속해서 개선했습니다. 실시간 캔들 차트, 호가창, 온디바이스 AI 속보 뉴스, 월별 랭킹 및 출석 보상 시스템 등 게임적 요소와 다양한 실시간 실시간 금융 인터랙션을 구현했습니다.',
  func: [
    '실시간 호가창 및 지정가/시장가 매매 주문',
    '실시간 캔들 차트 및 보조지표',
    '실제 시장 흐름을 시뮬레이션하는 가상 거래 봇',
    '주가 변동에 따른 실시간 AI 속보 뉴스 발행',
    '실시간 유저 자산 랭킹 및 7단계 등급 시스템',
    '매일 연속 출석 체크 보상 및 선물함',
    '보유 주주 전용 실시간 종목 채팅방',
