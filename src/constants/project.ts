  ...
  issues: [
    {
      ...
      solving: '테스트가 성공해야만 배포가 시작되는 **Quality Gate**를 구축했습니다. **Playwright**를 통한 핵심 유저 시나리오 E2E 테스트를 자동화하고, 브라우저 바이너리 캐싱 전략으로 CI 시간을 60% 단축했습니다. 모든 테스트 통과 시에만 배포용 워크플로우를 트리거하고, 각 플랫폼의 자체 자동 배포를 차단하여 오직 검증된 아티팩트만 배포되는 **Atomic Deployment**를 실현했습니다.'
    },
    {
