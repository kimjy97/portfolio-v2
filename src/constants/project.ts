...
export const projectData: IProjectProps[] = [{
  ...
  stacks: ['Next.js', 'React', 'TypeScript', 'NestJS', 'PostgreSQL', 'Prisma', 'Redis', 'TailwindCSS', 'Zustand', 'TanStack Query', 'Socket.IO', 'Docker', 'Cloudflare', 'Ollama'],
  issues: [
    {
      issue: '거래 데이터가 누적될수록 매번 전체 내역을 실시간 집계하여 차트 로딩이 8초 이상 지연되는 문제',
      solving: '캔들 차트 전용 테이블과 복합 인덱스를 설계하고, 거래 체결 시 분봉 데이터를 미리 계산해 두는 **사전 집계 파이프라인과 캐싱**을 도입했습니다. 그 결과 차트 조회 속도를 __8.6초에서 0.8ms로 단축__하고, __초당 300회의 요청을 안정적으로 처리__할 수 있도록 최적화했습니다.'
    },
    {
      issue: '봇과 유저들의 대량 주문이 몰릴 때, 한 번의 거래 처리에 너무 많은 작업이 묶여 데이터베이스 충돌(Deadlock)이 발생하는 문제',
      solving: '거래 즉시 처리해야 하는 **핵심 데이터(원장 기록, 잔고 반영)**와 나중에 처리해도 되는 **부가 작업(랭킹 계산, 칭호 부여)**을 분리하고, 봇 거래는 큐를 활용해 묶어서 처리하도록 개선했습니다. 이를 통해 대량 주문 상황에서도 __데이터베이스 충돌 발생률 0%__를 유지했습니다.'
    },
    {
      issue: '제한된 메모리 환경에서 대형 AI 모델 구동 시 뉴스 생성이 수십 초씩 지연되고, 서버 메모리 부족으로 시스템이 느려지는 문제',
      solving: '한국어 처리에 뛰어난 **경량 온디바이스 AI 모델(EXAONE 2.4B)**로 교체하고 뉴스 생성 프롬프트를 간결하게 최적화했습니다. 또한 컨테이너별 메모리 할당량을 재조정하여 __뉴스 생성 속도를 10초 이내로 단축__하고, __부족했던 가상 메모리(Swap) 사용량을 72%에서 0%로 완벽히 해소__했습니다.'
    },
    {
      issue: '24시간 봇 가동 시 주가가 비정상적으로 계속 상승하는 문제와, 봇의 잦은 계산 작업이 메인 서버를 점유해 유저의 웹 응답이 느려지는 문제',
      solving: '주가가 기준 가격에서 벗어나면 자연스럽게 매도 압력이 작용하는 **가격 균형 알고리즘**을 적용해 실제 시장과 같은 조정을 연출했습니다. 또한 봇을 별도 컨테이너로 분리하고 **Redis 메시지 큐 기반 비동기 통신**을 적용해 __초당 수백 회의 시뮬레이션 연산에도 메인 서버 부하 0%__를 달성하며 서비스를 안정적으로 격리했습니다.'
    },
    {
      issue: '가입 후 실제 거래로 이어지지 않고 일회성 방문에 그치기 쉬운 문제',
      solving: '직관적인 온보딩 인터페이스와 연속 출석 보상 시스템을 구축했습니다. 그 결과 가입자 대비 첫 거래 실행률 90.0%(18/20명), 출석 참여율 90.0%(최대 27일 연속 방문), **상점 아이템 구매 전환율 45.0%**를 기록했습니다. 또한 오픈베타 기간 동안 문의 게시판과 설문조사를 운영하며 사용자의 요구사항과 불편사항을 수집하고 서비스 개선에 반영했습니다.'
    },
    {
      issue: '주문 체결 알림이 여러 개 동시에 표시될 경우 토스트 UI가 주문창을 가려 거래를 방해하는 문제',
      solving: '오픈베타 기간 문의 게시판을 통해 주문 체결 토스트가 여러 개 쌓이면 주문창을 가린다는 사용자 피드백을 확인했습니다. 기존처럼 토스트를 수직으로 쌓는 대신 서로 겹쳐 표시되도록 UI를 변경하여 주문창을 가리지 않으면서도 체결 알림을 확인할 수 있도록 개선했습니다.'
    }
  ],
  reason: '`Next.js 16 App Router`와 `React 19`를 사용해 모바일과 데스크톱에서 사용할 수 있는 반응형 트레이딩 대시보드를 구축했습니다. 대량의 캔들 데이터를 효율적으로 렌더링하기 위해 `TradingView Lightweight Charts`를 사용했고, 백엔드는 `NestJS`, `PostgreSQL`, `Redis`를 기반으로 실시간 체결과 이벤트 처리를 구현했습니다. 24시간 동작하는 마켓 메이커 봇은 메인 서버와 분리해 서비스에 미치는 부하를 줄였으며, `Oracle Cloud ARM`과 `Docker` 환경에서 경량 AI 모델을 직접 서빙해 외부 API 비용을 줄였습니다.',
  learned: ' 주식 거래 게임을 처음부터 설계하고 개발해 오픈베타를 운영하면서, 데이터베이스 인덱싱과 사전 집계를 활용해 대량의 거래 데이터를 효율적으로 처리하는 방법을 경험했습니다. 또한 대량 주문이 발생하는 상황에서 핵심 작업과 부가 작업을 분리하고 비동기 처리를 적용하며 서비스의 안정성을 높이는 방법을 익혔습니다.\n 제한된 클라우드 자원에서는 컨테이너별 메모리를 조정하고 경량 AI 모델을 최적화하며 서버 자원을 효율적으로 사용하는 경험도 했습니다. 오픈베타 기간에는 문의 게시판과 설문조사를 통해 실제 사용자의 요구사항과 불편사항을 확인하고 UI와 기능을 개선하면서, 개발한 기능을 실제 사용자의 관점에서 다시 바라보는 경험을 할 수 있었습니다.',
  intro: '자체 개발한 마켓 메이커 시뮬레이션 엔진과 실시간 체결 엔진을 기반으로 만든 웹 기반 주식 거래 게임입니다. 실제 시장의 거래 흐름을 게임으로 구현하고, 오픈베타 테스트(OBT)를 운영하며 사용자 피드백과 데이터를 바탕으로 서비스를 지속적으로 개선했습니다. 실시간 캔들 차트와 호가창, 주문 기능을 비롯해 AI 속보 뉴스, 월별 랭킹, 출석 보상 등 게임 요소를 결합했습니다.',
  func: [
    '실시간 호가창 및 지정가/시장가 주문',
    '실시간 캔들 차트 및 보조지표',
    '마켓 메이커 알고리즘 기반 가상 거래 봇',
    '주가 변동에 따른 실시간 AI 속보 뉴스',
    '실시간 유저 자산 랭킹 및 7단계 등급 시스템',
    '연속 출석 보상 및 선물함',
    '보유 주주 전용 실시간 종목 채팅',
    '모바일 최적화 반응형 디자인 및 다크 모드'
  ]
}, {
  ...
  issues: [
    {
      issue: '코드 수정 시 예상치 못한 오류가 걸러지지 않고 실서버에 그대로 배포될 수 있는 문제',
      solving: '**Playwright**로 로그인부터 메모 작성, 실시간 동시 편집까지 핵심 이용 흐름을 자동으로 검증하는 **E2E 테스트를 자동화**했습니다. 브라우저 환경 캐싱으로 검사 시간을 60% 단축하고, 모든 테스트를 통과해야만 배포가 이뤄지도록 파이프라인을 구축해 결함 있는 코드가 배포되는 사고를 사전에 방지했습니다.'
    },
    {
      issue: '서버리스 배포 환경 특성상 요청이 끝나면 서버가 꺼져, 실시간 협업에 필요한 웹소켓 연결이 자꾸 끊어지는 문제',
      solving: '웹 화면과 API는 Vercel에 배포하고, 지속적인 연결이 필요한 실시간 기능은 상시 켜져 있는 **Railway 소켓 서버**로 분리했습니다. 또한 사용자가 글을 쓸 때마다 매번 DB에 요청하지 않고 메모리에 모았다가 주기적으로 한 번에 저장하도록 개선해, 데이터베이스 부담을 크게 줄이고 부드러운 실시간 동시 편집을 구현했습니다.'
    },
    {
      issue: '단순 텍스트 검색만으로는 오타나 비슷한 의미의 메모를 찾기 어렵고, 메모가 많아질수록 검색 속도가 느려지는 문제',
      solving: '정확한 단어를 찾는 **키워드 검색**과 문맥의 의미를 파악하는 **AI 벡터 검색**을 결합했습니다. 두 검색 결과의 연관도를 균형 있게 계산하는 **순위 재조정 알고리즘**을 자체 구현하고 전용 인덱스를 적용하여, 수많은 메모 속에서도 오타나 유사 표현까지 **밀리초 단위로 정확하게 찾아내도록 최적화**했습니다.'
    },
    {
      issue: '메모를 저장하거나 수정할 때마다 매번 고비용 AI 임베딩 API를 호출해 비용과 서버 부하가 커지는 문제',
      solving: '메모 내용의 해시값을 비교해 **실제 본문이 변경된 메모만 선별**하도록 개선했습니다. 또한 백그라운드 큐를 활용해 변경된 메모들을 일정 주기로 묶어서 한 번에 처리함으로써 불필요한 호출을 걸러내고 **AI API 호출 비용을 대폭 절감**했습니다. 분석이 끝난 메모에는 AI가 자동으로 카테고리 태그까지 추천하도록 연계했습니다.'
    },
    {
      issue: '외부 AI 서비스에 일시적인 장애나 응답 지연이 발생하면 메모 요약이나 태깅 같은 핵심 기능이 멈추는 문제',
      solving: '외부 AI 호출이 실패하거나 지연되면 즉시 대기 중인 **로컬 AI 모델로 우회하는 폴백 시스템**을 구축했습니다. 클라우드 API와 로컬 모델을 결합한 **이중화 구조**를 통해 외부 장애 상황에서도 핵심 AI 기능이 중단되지 않고 안정적으로 동작하도록 했습니다.'
    }
  ],
